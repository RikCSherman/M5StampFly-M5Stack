...
void setup_pwm_buzzer(void) {
    ledcSetup(channel, 4000, 8);        // 配置PWM通道：通道0，频率3000Hz，分辨率8位 Configure PWM channel: channel 0, frequency 3000Hz, resolution 8 bits
    ledcAttachPin(buzzerPin, channel);  // 将PWM通道绑定到GPIO Attach PWM channel to GPIO
}
